...
class VGGTransferModel(nn.Module):
    def __init__(self, model_type, pretrained):
        ...
        if model_type == "VGG16":
            self.feature_dim = 25088    # 512*7*7

        self.FC_hair = FullConnect(input_dim=self.feature_dim)
        self.FC_hair_color = FullConnect(input_dim=self.feature_dim, output_dim=5)
    ...
    def forward(self, image):
        ...
        hair_color=self.FC_hair_color(features)
        ...
        return hair, hair_color, gender, earring, smile, frontal, style
